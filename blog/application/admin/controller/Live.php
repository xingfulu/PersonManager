...
{  
	...
	public function add(){
		 if(request()->isPost()) {//j检测是否是post提交
            ...
            if($res['valid']){
                //操作成功
                $this->success($res['msg'],'birthday');exit;
            }else{
    ...
    //修改生日日历
     ...
     public function del(){
         //此处接受到页面上删除字段的id值
       $birthday_id=input('get.birthday_id');//如果不是用框架，应使用param.famlily_id
       //halt($famlily_id);
       //下面请求模型中的del方法来删除,去模型中建立一个对应的模块del去进行删除
       $res=$this->db->del(input('get.birthday_id'));
       if($res['valid']){
           $this->success($res['msg'],'birthday');exit;
       }else{
           $this->error($res['msg']);exit;
