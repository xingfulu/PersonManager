<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"C:\xampp\htdocs\blog\public/../application/admin\view\study\study.html";i:1497926820;s:63:"C:\xampp\htdocs\blog\public/../application/admin\view\base.html";i:1498183363;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>后台管理页面</title>
...
<caption class="Myhead1">学习计划表</caption>
<button class="btn btn-success btn-study" onclick="add()">增加学习任务</button>
<span class="Todaydate"></span>
 ...
 <?php if(is_array($field) || $field instanceof \think\Collection): if( count($field)==0 ) : echo "" ;else: foreach($field as $key=>$vo): ?>
 	<tr>
 		<td><?php echo $vo['study_content']; ?></td>
 		<td><?php echo $vo['study_time']; ?></td>
 		<td><?php echo $vo['study_result']; ?></td>
 		<td>
 			<button class="btn btn-info btn-xs" onclick="update(<?php echo $vo['study_id']; ?>)">编辑</button>
 		</td>
 		<td>
 			<button class="btn btn-danger btn-xs" onclick="del(<?php echo $vo['study_id']; ?>)">删除</button>
 		</td>
 	</tr>
...
//修改学习任务
	function update(study_id){	
     	location.href = "<?php echo url('update'); ?>"+'?study_id='+study_id;  
	}
//删除任务
function del(study_id){	
		var confireM=confirm('确定删除吗?');
        if(confireM){
            location.href = "<?php echo url('del'); ?>"+'?study_id='+study_id;
        }
	}
</script>
</div>
