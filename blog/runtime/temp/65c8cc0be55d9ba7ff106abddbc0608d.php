<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"C:\xampp\htdocs\blog\public/../application/admin\view\live\birthday.html";i:1497922884;s:63:"C:\xampp\htdocs\blog\public/../application/admin\view\base.html";i:1498183363;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>后台管理页面</title>
...
<button class="btn btn-success btn-lg MyBtn" onclick="add()">增加生日记录</button>
<?php if(is_array($field) || $field instanceof \think\Collection): if( count($field)==0 ) : echo "" ;else: foreach($field as $key=>$vo): ?>
<div class="MyBir">
	<button class="btn btn-success" onclick="update(<?php echo $vo['birthday_id']; ?>)">修改</button>
	<h1><?php echo $vo['birthday_name']; ?></h1>
	<span><?php echo $vo['birthday_day']; ?></span>
	<button class="btn btn-success" onclick="del(<?php echo $vo['birthday_id']; ?>)">删除</button>
</div>
...
<script type="text/javascript">
	...
	function update(birthday_id){
		var confireM=confirm('确定修改吗?');
        if(confireM){
            location.href = "<?php echo url('update'); ?>"+'?birthday_id='+birthday_id;
        }
	}
	function del(birthday_id){
		var confireM=confirm('确定删除吗?');
        if(confireM){
            location.href = "<?php echo url('del'); ?>"+'?birthday_id='+birthday_id;
        }
	}
</script>
       </div>
