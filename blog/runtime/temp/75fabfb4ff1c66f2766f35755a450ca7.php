<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:71:"C:\xampp\htdocs\blog\public/../application/admin\view\family\index.html";i:1497628679;s:63:"C:\xampp\htdocs\blog\public/../application/admin\view\base.html";i:1498183363;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>后台管理页面</title>
...
<div class="col-sm-10 col-xs-10 col-md-10 col-lg-10 WrapperWhole">
            ...
            <ul class="nav nav-tabs" role="tablist">
                <li class="active"><a href="">家属信息列表</a></li>
                <li><a href="<?php echo url('store'); ?>">添加家庭成员</a></li>
            </ul>
                    ...
                    <div class="panel-body edit">
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <th>与本人关系</th>
                                <th>工作单位</th>
                                <th colspan="2" class="MyControl">操作</th>
                            </tr>
                            </thead>
                            <tbody>
                            <!--在此处循环我们数据库中的数据,数据库中有多少条数据，就会循环出来多少条数据-->
                            <!--key为循环中的键名，vo为循环中的键值，name指定我们要循环的数据，和Family控制器中的名相对应-->
                            <?php if(is_array($field) || $field instanceof \think\Collection): if( count($field)==0 ) : echo "" ;else: foreach($field as $key=>$vo): ?>
                                <tr>
                                    <td><?php echo $vo['famlily_id']; ?></td>
                                    <td><?php echo $vo['famlily_name']; ?></td>
                                    <td><?php echo $vo['famlily_relation']; ?></td>
                                    <td><?php echo $vo['famlily_job']; ?></td>
                                    <td>
                                            <button class="btn btn-primary btn-xs ">
                                                <a href="<?php echo url('edit',['famlily_id'=>$vo['famlily_id']]); ?>">编辑</a>
                                            </button>
                                    </td>  
                                    <td>
                                        <button  class="btn btn-primary btn-xs ">
                                            <a href="javascript:;" onclick="del(<?php echo $vo['famlily_id']; ?>)">删除</a>
                                        </button>
                                    </td>
                                </tr>
...
<script>
    function del(famlily_id) {
        var confireM=confirm('确定删除吗?');
        if(confireM){
            location.href = "<?php echo url('del'); ?>"+'?famlily_id='+famlily_id;
        }
    }
</script>
       </div>
