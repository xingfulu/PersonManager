<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:68:"C:\xampp\htdocs\blog\public/../application/admin\view\show\show.html";i:1498143312;s:63:"C:\xampp\htdocs\blog\public/../application/admin\view\base.html";i:1498183363;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>后台管理页面</title>
...
<caption class="Myhead1">个人作品</caption>
<button class="btn btn-success btn-study" onclick="add()">增加作品</button>
<span class="Todaydate"></span>
 ...
 <?php if(is_array($field) || $field instanceof \think\Collection): if( count($field)==0 ) : echo "" ;else: foreach($field as $key=>$vo): ?>
 	<tr class="MyShow">
 		<td><?php echo $vo['show_head']; ?></td>
 		<td><?php echo $vo['show_content']; ?></td>
 		<td><?php echo $vo['show_address']; ?></td>
 		<td>
 			<button class="btn btn-info btn-xs" onclick="update(<?php echo $vo['show_id']; ?>)">编辑</button>
 		</td>
 		<td>
 			<button class="btn btn-danger btn-xs" onclick="del(<?php echo $vo['show_id']; ?>)">删除</button>
 		</td>
 	</tr>
...
//修改学习任务
	function update(show_id){	
     	location.href = "<?php echo url('update'); ?>"+'?show_id='+show_id;  
	}
//删除任务
function del(show_id){	
		var confireM=confirm('确定删除吗?');
        if(confireM){
            location.href = "<?php echo url('del'); ?>"+'?show_id='+show_id;
        }
	}
</script>
</div>
